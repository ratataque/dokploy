							...
							<div className="flex flex-col gap-2">
																...
																>
																	...
																	<SelectContent>
																		{services?.map((service, index) => (
																			<SelectItem value={service} key={`${service}-${index}`}>
																				{service}
																			</SelectItem>
																		))}
																...
																<>
																	...
																	<TooltipProvider delayDuration={0}>
																		<Tooltip>
																			...
																			>
																				<p>
																					Cache: If you previously deployed this compose, it will
																					read the services from the last deployment/fetch from the
																					repository
																				</p>
																			</TooltipContent>
																		</Tooltip>
								...
								{advanced && (
									<div className="mt-6 p-4 border rounded-lg bg-muted/50">
										<h4 className="text-sm font-medium mb-4">Advanced Configuration</h4>
											...
											<FormField
												...
												render={({ field }) => (
													<FormItem>
														...
														<Select
															onValueChange={value => {
																const currentMiddlewares = field.value || [];
																if (!currentMiddlewares.includes(value)) {
																	field.onChange([...currentMiddlewares, value]);
																}
															}}
														>
															<FormControl>
																<SelectTrigger>
																	<SelectValue placeholder="Select middlewares to add" />
																</SelectTrigger>
															</FormControl>
															<SelectContent>
														...
														<FormDescription>
															Configure Traefik middlewares for this domain
														</FormDescription>
														{field.value && field.value.length > 0 && (
															<div className="flex flex-wrap gap-2 mt-2">
																{field.value.map(middlewareId => {
																	const middleware = availableMiddlewares?.find(
																		m => m.id === middlewareId
																	);
																	return (
																		<div
